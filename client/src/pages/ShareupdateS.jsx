...
export default function ShareupdateS() {
  ...
  useEffect(() => {
    try {
      const fetchworkout = async () => {
        const res = await fetch(`http://localhost:8081/api/Workoutt?itemId=${workkId}`);
        ...
        if (res.ok) {
            const selected = data.find((workout) => workout.id === workkId);
            if (selected) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8081/api/WorkoutUpp/${formData.id}`, {
        method: "PUT",
        headers: {
      ...
      <div className="flex p-3 max-w-3xl mx-auto flex-col md:flex-row md:items-center gap-5">
        ...
        <div className="flex justify-center items-center mt-6">
          <form className="flex flex-col  gap-4" onSubmit={handleSubmit} >
            ...
            <div>
            <h3 className="font-semibold text-gray-600 ml-1">WorkoutState</h3>
            <select
          ...
          >
            ...
            <div>
             <h3 className="font-semibold text-slate-700 ml-1 ">Date</h3>
            ...
            {Error && (
            <p className="mt-5 text-red-800 font-serif text-opacity-50  w-300 rounded-lg text-center ">
              {Error}
            </p>)}

            <div>
             <h3 className="font-semibold text-gray-600 ml-1">Description</h3>
